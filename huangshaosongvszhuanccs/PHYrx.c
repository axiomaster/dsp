...
/*---------------------------------接收端，PLCP解调译码--------------------------------------*/
		...
		//printf("从fpga接收来的一帧的PPDU_Mod数组长度为:%d\n",Mod_len); //1286
		/*for(i=0;i<Mod_len;i++)
		...
		PLCP_DeMod_SoftInfo = DPSK_DeMod_SoftInfo(PLCP_Mod_R,PLCP_LEN,PLCP_H_M); //软解调
		//printf("PLCP解调之后长度为：%d\n PLCP解调之后输出为：\n", PLCP_LEN);  //214（PLCP的长度一直都不变）
		/*for(i=0;i<PLCP_LEN;i++)
			printf("%f ",PLCP_DeMod_SoftInfo[i]);
		printf("\n");*/
...
/*---------------------------------接收端，PSDU解调译码--------------------------------------*/
		...
		PSDU_Mod_R = PPDU_Mod + PLCP_Mod_len; //PSDU_Mod_R和PPDU_Mod是复数类型的指针，PLCP_Mod_len是长度
		...
		PSDU_DeMod_SoftInfo = DPSK_DeMod_SoftInfo(PSDU_Mod_R,PSDU_DeMod_len,PSDU_M_R); //PSDU软解调
